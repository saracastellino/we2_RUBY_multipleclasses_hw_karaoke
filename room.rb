...
attr_reader :name, :available_places, :playlist, :till, :entry_fee, :guests
 ...
 def initialize(name, available_places, playlist, till, entry_fee, guests)
  ...
  def check_out_guest(guest)
   ...
   def deny_access_if_room_full(*guest)
     check_in_guest(*guest) if @available_places > 0 && guest.wallet >= 5.50
   end

   def fav_song_on_air(guest)
     @playlist.each {|song| puts song == guests.fav_song ? "Yay!!! Give me the mic NOW!!!" : "Cool!"}
   end
